...
import { IconMosque, IconBook, IconCalendar, IconLock, IconUsers, IconBrandFacebook } from '@/components/ui/Icons'
...
export default function AboutPage() {
    ...
    const fbContent = {
        no: {
            sectionTitle: 'Følg oss',
            desc: 'Ålesund Moskè er et aktivt trossamfunn som driver mange forskjellige aktiviteter og er et registrert trossamfunn i Norge. Følg oss for oppdateringer, arrangementer og kunngjøringer.',
            btn: 'Følg oss på Facebook',
        },
        en: {
            sectionTitle: 'Follow us',
            desc: 'Ålesund Mosque is an active faith community running various activities and is a registered religious organisation in Norway. Follow us for updates, events and announcements.',
            btn: 'Follow us on Facebook',
        },
        ar: {
            sectionTitle: 'تابعنا',
            desc: 'مسجد أولسند مجتمع ديني نشط يُنظّم أنشطة متنوعة، وهو منظمة دينية مسجّلة رسمياً في النرويج. تابعونا للاطلاع على آخر الأخبار والفعاليات والإعلانات.',
            btn: 'تابعنا على فيسبوك',
        },
    }

    const c = community[lang]
    const fb = fbContent[lang]
    ...
    return (
        <div style={{ maxWidth: '1120px', margin: '0 auto', padding: isMobile ? '40px 20px' : '64px 40px' }}>
            ...
            <div style={{ marginTop: '40px', direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
                <div style={{ fontSize: '19px', fontWeight: 700, color: '#f0f4f8', letterSpacing: '-0.3px', marginBottom: '20px' }}>
                    {fb.sectionTitle}
                </div>

                <div
                    style={{ background: '#111e2d', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '20px', overflow: 'hidden', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'stretch', transition: 'all 0.2s' }}
                    onMouseEnter={e => e.currentTarget.style.borderColor = 'rgba(22,101,52,0.28)'}
                    onMouseLeave={e => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)'}>

                    <div style={{ width: isMobile ? '100%' : '160px', minHeight: isMobile ? '120px' : 'auto', flexShrink: 0, background: 'rgba(22,101,52,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <div style={{ width: '72px', height: '72px', borderRadius: '50%', background: 'rgba(22,101,52,0.14)', border: '2px solid rgba(22,101,52,0.28)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#22a052' }}>
                            <IconBrandFacebook size={32} />
                        </div>
                    </div>

                    <div style={{ padding: '20px 24px', flex: 1, textAlign: lang === 'ar' ? 'right' : 'left' }}>
                        <div style={{ fontSize: '16px', fontWeight: 700, color: '#f0f4f8', marginBottom: '6px' }}>
                            Ålesund Moske
                        </div>
                        <div style={{ display: 'flex', gap: '16px', marginBottom: '12px', justifyContent: isMobile ? 'center' : 'flex-start', flexWrap: 'wrap' }}>
                            {[
                                { value: '1K', label: lang === 'ar' ? 'متابع' : lang === 'no' ? 'følgere' : 'followers' },
                                { value: '297', label: lang === 'ar' ? 'منشور' : lang === 'no' ? 'innlegg' : 'posts' },
                            ].map(stat => (
                                <div key={stat.label} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                    <span style={{ fontSize: '13px', fontWeight: 700, color: '#22a052' }}>{stat.value}</span>
                                    <span style={{ fontSize: '12px', color: '#607080' }}>{stat.label}</span>
                                </div>
                            ))}
                        </div>
                        <p style={{ fontSize: '13px', color: '#a8b8c8', lineHeight: 1.65, marginBottom: '16px', maxWidth: '480px' }}>
                            {fb.desc}
                        </p>
        ...
        <div style={{ maxWidth: '1120px', margin: '0 auto', padding: isMobile ? '40px 20px' : '64px 40px' }}>
            ...
            <div style={{ marginTop: '40px', direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
                ...
                <div
                    ...
                    onMouseLeave={e => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)'}
                >
                    {/* Icon column — same width as women's group image */}
                    <div style={{ width: isMobile ? '100%' : '200px', height: isMobile ? '120px' : 'auto', flexShrink: 0, background: 'rgba(22,101,52,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <div style={{ width: '72px', height: '72px', borderRadius: '50%', background: 'rgba(22,101,52,0.14)', border: '2px solid rgba(22,101,52,0.28)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#22a052' }}>
                            <IconBrandFacebook size={32} />
                        </div>
                    </div>
                    ...
                    {/* Content */}
                    <div style={{ padding: '20px 24px', flex: 1 }}>
                        {/* Stats badges — same style as women's group */}
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '10px' }}>
                            <span style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.8px', textTransform: 'uppercase', color: '#22a052', background: 'rgba(22,101,52,0.14)', border: '1px solid rgba(22,101,52,0.28)', padding: '4px 10px', borderRadius: '20px' }}>
                                {fb.followers}
                            </span>
                            <span style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.8px', textTransform: 'uppercase', color: '#607080', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', padding: '4px 10px', borderRadius: '20px' }}>
                                {fb.posts}
                            </span>
                        </div>
                        <div style={{ fontSize: '16px', fontWeight: 700, color: '#f0f4f8', marginBottom: '8px' }}>{fb.name}</div>
                        <p style={{ fontSize: '13px', color: '#a8b8c8', lineHeight: 1.65, marginBottom: '16px' }}>{fb.desc}</p>
                        <a href="https://www.facebook.com/share/1FU3p6YUzA/"
                            target="_blank"
                            rel="noopener noreferrer"
                            style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', background: 'rgba(22,101,52,0.14)', border: '1px solid rgba(22,101,52,0.28)', color: '#22a052', fontSize: '13px', fontWeight: 700, padding: '9px 16px', borderRadius: '10px', textDecoration: 'none' }}
