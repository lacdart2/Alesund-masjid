...
import { scheduleAdhanNotifications, cancelAdhanNotifications } from '@/lib/scheduleAdhan'
...
const isCapacitorEnv = () => typeof window !== 'undefined' && typeof (window as any).Capacitor !== 'undefined'
...
export default function PrayerNotification() {
    ...
    const [selectedAdhan, setSelectedAdhan] = useState('abdul-rahman')
    ...
    // ✅ Re-schedule on app start if already enabled on Android
    useEffect(() => {
        if (isCapacitorEnv() && enabled) {
            scheduleAdhanNotifications(lang)
        }
    }, [enabled, lang])
    ...
    const requestPermission = () => {
        if (isCapacitorEnv()) {
            // On Android — use Capacitor local notifications directly
            scheduleAdhanNotifications(lang).then(() => {
                setEnabled(true)
    ...
    const handleDisable = () => {
        ...
        // ✅ Cancel all scheduled notifications on Android
        if (isCapacitorEnv()) cancelAdhanNotifications()
    }
    ...
    const handleSave = () => {
        ...
        setModalOpen(false)
        // ✅ Re-schedule with updated settings on Android
        if (isCapacitorEnv() && enabled) {
            scheduleAdhanNotifications(lang)
        }
    }
    ...
    if (typeof Notification === 'undefined' && !isCapacitorEnv()) return null
                    ...
                    <div
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#162538', borderRadius: '14px', padding: '16px', marginBottom: '16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <span style={{ color: enabled ? '#22a052' : '#607080', display: 'flex' }}><IconBell size={20} /></span>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                {PRAYERS.map(key => (
                                    <div key={key} style={{
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            {prayerToggles[key]
                                                ? <span style={{ color: '#22a052', display: 'flex' }}><IconBell size={18} /></span>
                                                : <span style={{ color: '#607080', display: 'flex' }}><IconBellOff size={18} /></span>
                                            }
                        ...
                        <button
                            onClick={handleSave}
                            style={{
                                marginTop: '16px', width: '100%', padding: '16px', borderRadius: '14px',
